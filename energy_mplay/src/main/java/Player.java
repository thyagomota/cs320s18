...
class Player {
    ...
    // TODOd: add the @SpaceId annotation to define the field that identifies the object at the space
    @SpaceId
    ...
    public void move() {
        // TODOf: if player is not moving, increase its energy by 1 but only up to MAXIMUM_ENERGY; if player is moving but its energy level reached 0, have the player stop; else, decrease the energy level by 1, not allowing it to become negative
        if (direction == NONE) {
            energy++;
    ...
    // TODOd: implement collision detection
